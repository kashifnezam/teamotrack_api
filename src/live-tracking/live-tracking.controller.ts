...
export class LiveTrackingController {
  ...
  history(@CurrentUser() user: any, @Body() dto: LiveTrackingDto) {
    return this.service.getHistory(user.uid, dto);
  }

  // Current live employees
  @Get('data')
  @UseGuards(FirebaseAuthGuard)
  live(@CurrentUser() user: any) {
    return this.service.getLive(user.uid);
  }
}
