...
export class ManagersController {
  ...
  get(@CurrentUser() user: any) {
    return this.service.getAll(user.uid, 'manager');
  }
  ...
  create(@CurrentUser() user: any, @Body() dto: StaffDto) {
    return this.service.create(user.uid, 'manager', dto);
  }

  @Get('parents')
  @UseGuards(FirebaseAuthGuard)
  parents(@CurrentUser() user: any) {
    return this.service.getParents(user.uid, 'manager');
  }

  @Patch(':id')
  @UseGuards(FirebaseAuthGuard)
  update(@CurrentUser() user: any, @Param('id') id: string, @Body() dto: StaffDto) {
