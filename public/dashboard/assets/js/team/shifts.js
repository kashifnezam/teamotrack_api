...
async function initializeShiftsPage() {
...
function editShift(id) {
    ...
    document.getElementById("graceMinutes").value =
        shift.graceMinutes || 0;

    document.getElementById("halfDayMinutes").value =
        shift.halfDayMinutes || 0;

    document.getElementById("fullDayMinutes").value =
        shift.fullDayMinutes || 0;

    const weeklyOff = shift.weeklyOff || [];

    document
        .querySelectorAll("#shiftModal .weekly-off input")
        .forEach(input => {

            const value = Number(input.value);

            input.checked =
                weeklyOff.includes(WEEK_DAYS[value]) ||
                weeklyOff.includes(value);
...
async function deleteShift(id) {
    ...
    const result =
        await Swal.fire({
            ...
            text:
                `"${shift.name}" will be permanently deleted.`,
...
function formatTime(hour, minute) {
    ...
    return `${String(hour || 0).padStart(2, "0")}:${String(
        minute || 0
    ).padStart(2, "0")}`;
