(function () {
        ...
        const gross =
            records.reduce(
                (
                    ...
                    total +
                    Number(
                        item.salary?.payableGross ||
                        item.salary?.grossSalary ||
                        0,
                    ),
                0,
            );
        ...
        const deduction =
            records.reduce(
                (
                    ...
                    total +
                    Number(
                        item.salary?.payableDeduction ||
                        item.salary?.totalDeduction ||
                        0,
                    ),
                0,
            );
    ...
    function render() {
        ...
        if (
            ...
            $('payrollCalculationEmpty')
                .classList
                .remove('d-none');

            $('payrollCalculationTableWrap')
                .classList
                .add('d-none');

            $('payrollCalculationMobileList')
                .classList
                .add('d-none');
    ...
    async function loadCalculations(
        ...
        try {
            ...
            const response =
                await Api.get(
                    `/payroll-calculations/data/${periodId}`,
                );
            ...
            AppAlert.error(
                error?.message ||
                'Failed to load payroll calculations',
            );
    ...
    async function calculatePayroll() {
        ...
        try {
            ...
            AppAlert.success(
                `Payroll calculated for ${response?.totalEmployees ||
                records.length
                } employees`,
            );
    ...
    window.initializePayrollCalculationsPage =
    async function () {
