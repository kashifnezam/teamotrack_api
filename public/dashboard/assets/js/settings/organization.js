...
(function () {
  ...
  async function loadOrganizationData() {
    try {
      data = await Api.get('/settings/organization');
       localStorage.setItem('company', JSON.stringify(data));
    } catch (error) {
      AppAlert.close();

      AppAlert.error(error?.message || 'Unable to load organization data');
    }
  }
  ...
  function renderCompany() {
    ...
    const fields = {
      ...
      organizationPhone: company.mobile,
  ...
  window.saveCompany = async function () {
    ...
    const dto = {
      ...
      mobile: document.getElementById('organizationPhone')?.value.trim(),
